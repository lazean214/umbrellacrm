...
<section x-data="{ expanded: true }"
    class="bg-gray-100 text-black rounded-lg border dark:border-slate-700 p-4 dark:bg-slate-800 dark:text-slate-100 mt-4">
    <h2 class="text-sm uppercase font-bold flex justify-between">MDA
        <span class="flex items-center space-x-2">
            <button wire:click="syncToMDA"
                class="bg-indigo-500 text-xs text-white px-4 py-2 rounded-lg hover:bg-indigo-600 transition">Sync to
                MDA</button>
    ...
    <div x-show="expanded" x-collapse.duration.300ms>
        <label class="text-xs font-bold uppercase tracking-wider">
            ...
            <select wire:model="mda_setup"
                class="capitalize block w-full pl-4 pr-3 py-2 text-sm bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500 transition mb-2">
                <option value="" disabled selected>Select MDA Setup</option>
                @foreach ($internalCompanies as $company)
                    <option value="{{ $company['name'] }}" class="text-black dark:text-slate-100">{{ $company['name'] }}
                    </option>
                @endforeach
            </select>
        </label>
        ...
        <input type="text" wire:model="mda_reference_number"
            class="block w-full pl-4 pr-3 py-2 text-sm bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500 transition mb-2" />
