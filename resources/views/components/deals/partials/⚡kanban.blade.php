...
<div x-data="kanbanBoard({
    ...
    <template x-for="stage in stages" :key="stage">
        <section :data-stage="stage"
            ...
            x-on:dragenter.prevent x-on:dragover.prevent="canEditStage(stage) && onDragOver($event, stage)"
            x-on:dragleave="onDragLeave()" x-on:drop.prevent="canEditStage(stage) && onDrop(stage)">
            <header
                ...
                <div class="flex shrink-0 items-center gap-1.5">
                    <span class="rounded-full bg-black/20 px-2 py-0.5 text-xs font-normal tabular-nums text-white/80"
            ...
            <div
                class="sticky top-[60px] z-10 border-b border-slate-200 bg-white/70 px-4 py-2 text-xs font-semibold backdrop-blur-sm dark:border-slate-700 dark:bg-slate-800/40">
            ...
            <div :id="'kanban-col-' + stage" class="flex-1 space-y-2 overflow-y-auto p-2 md:p-3">
                <template x-if="getDealsByStage(stage).length === 0">
                    <div
                        class="flex h-full select-none flex-col items-center justify-center text-slate-400 dark:text-slate-600">
                ...
                <template x-for="deal in getDealsByStage(stage)" :key="deal.id">
                    <<<<<<< HEAD <article :data-deal-id="deal.id"
                        ...
                        :class="{ 'opacity-60': draggingId === deal.id }" :draggable="canEditStage(stage)"
                        x-on:dragstart.stop="onDragStart(deal.id, stage, $event)" x-on:dragend="resetDrag()"
                        x-on:click="openDeal($event, deal.id)">
                        <div class="absolute bottom-3 left-0 top-3 w-[3px] rounded-r-full"
                            :style="'background-color:' + (stageConfig[deal.stage]?.accent ?? '#94a3b8')"></div>
                        ...
                        <div class="min-w-0 flex-1 pl-1">
                            <p class="mb-2 truncate text-sm font-semibold text-slate-900 transition hover:text-indigo-600 dark:text-slate-100 dark:hover:text-indigo-400"
                                x-text="deal.name"></p>

                            <div class="mb-3 flex items-center justify-between gap-2">
                                <span class="text-sm font-bold tabular-nums text-slate-800 dark:text-white"
                                    x-text="'\u00a3' + Number(deal.amount || 0).toLocaleString('en-GB', { maximumFractionDigits: 0 })"></span>
                                <template x-if="deal.internal_company">
                                    <span
                                        class="max-w-[80px] shrink-0 truncate rounded border border-slate-200 bg-slate-50 px-1.5 py-0.5 text-[10px] font-semibold text-slate-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-400"
            ...
            <template x-if="isSalesUser && !canEditStage(stage)">
                <div
                    class="rounded-b-2xl border-t border-amber-100 bg-amber-50 px-3 py-2 text-center text-[11px] text-amber-700 dark:border-amber-900/30 dark:bg-amber-900/20 dark:text-amber-300">
        ...
        <div :data-deal-id="deal.id" class="transition-transform duration-200"
            ...
            x-on:dragstart.stop="canEditStage(stage) && onDragStart(deal.id, stage, $event)" x-on:dragend="resetDrag()">
        ...
        return {
            ...
            function kanbanBoard({
                ...
                return {
                    ...
                    >>>
                    >
                    2e63 ca614e8ce820dd4ded4c7c30f6ddc83b383c
                    draggingId: null,
                    draggingStage: null,
                    dragOverStage: null,
                                    ...
                                    try {
                                        ...
                                        if (card) {
                                            ...
                                            const shake = [0, -6, 6, -4, 4, 0]
                                                .map((x, i) => setTimeout(() => card.style.transform =
                                                    `translateX(${x}px)`, i * 60));
