    ...
    function kanbanBoard({
        ...
        return {
            ...
            init() {
                ...
                this.$watch('$wire.deals', (fresh) => {
                    if (fresh && fresh.length > 0) {
                        // Fast shallow equality check before invoking expensive map merges
            ...
            handleNewDeal(event) {
                ...
                // Check if item already exists locally to prevent duplicate cards
                if (!this.deals.some(d => d.id === newDeal.id)) {
                    // Push directly to top of deck
                    this.deals.unshift(newDeal);
                    writeCache(this.deals); // Update client-side cache
                }
            },
