...
new class extends Component {
    ...
    */
    ...
    public $stages = [DealStage::DOC_SENT, DealStage::DOC_SIGNED, DealStage::COMPLIANT, DealStage::READY_FOR_PAYMENT, DealStage::PAID];

    public function mount(int $dealId)
    {
        $this->dealId = $dealId;

        $this->internalCompanies = [['id' => 1, 'name' => InternalCompany::UMBRELLACOMPANY->value], ['id' => 2, 'name' => InternalCompany::CHURCHILL_KNIGHT_UMBRELLA->value], ['id' => 3, 'name' => InternalCompany::CHURCHILL_KNIGHT_ASSOCIATES->value]];
...
>>>>>>> 2e63ca614e8ce820dd4ded4c7c30f6ddc83b383c
    ...
    public function save(): void
    {
            ...
            if ($originalContact) {
                ...
                foreach ($contactFields as $field) {
                    if ($contact->$field != $originalContact->$field) {
                        $this->deals->logFieldUpdate("contact_{$field}", $originalContact->$field, $contact->$field, "Contact {$field} changed from \"{$originalContact->$field}\" to \"{$contact->$field}\"");
                    }
                }
            }
        }
        ...
        if (!empty($this->compliance_documents)) {
            ...
            // Log document upload
            $this->deals->logFieldUpdate('compliance_documents', 'No documents', count($this->compliance_documents) . ' document(s) uploaded', count($this->compliance_documents) . ' compliance document(s) uploaded');
        }
        ...
        if (!empty($this->contract_documents)) {
            ...
            // Log document upload
            $this->deals->logFieldUpdate('contract_documents', 'No documents', count($this->contract_documents) . ' document(s) uploaded', count($this->contract_documents) . ' contract document(s) uploaded');
        }
    ...
    //Sync to MDA functionality will be implemented here in the future add dummy function for now
    public function syncToMDA()
    {
        // This is where the logic to sync data to MDA would go
        // For now, we can just simulate a successful sync with a session flash message
...
<div wire:poll.5s="loadDeal">
    ...
    <div class="flex flex-wrap">
        ...
        <main class="w-4/6 px-5">
            <section class="">
                <div class="mb-6">
                    <div
                        ...
                        {{-- Overview --}}
                        <button wire:click="$set('openTabs', 'overview')" @class([
                            'flex-1 rounded-xl px-5 py-3 text-sm font-semibold transition-all duration-300',
                            'bg-indigo-500 text-white shadow-md' => $openTabs === 'overview',
                        ...
                        {{-- Activities --}}
                        <button wire:click="$set('openTabs', 'activities')" @class([
                            'flex-1 rounded-xl px-5 py-3 text-sm font-semibold transition-all duration-300',
                            'bg-indigo-500 text-white shadow-md' => $openTabs === 'activities',
                            'text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 hover:bg-white/60 dark:hover:bg-slate-700/50' =>
                                $openTabs !== 'activities',
                        ])>
                            <div class="flex items-center justify-center gap-2">
                        ...
                        {{-- Welcome Email --}}
                        <button wire:click="$set('openTabs', 'email')" @class([
                            'flex-1 rounded-xl px-5 py-3 text-sm font-semibold transition-all duration-300',
                            'bg-indigo-500 text-white shadow-md' => $openTabs === 'email',
                            'text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 hover:bg-white/60 dark:hover:bg-slate-700/50' =>
                                $openTabs !== 'email',
                        ])>
                            <div class="flex items-center justify-center gap-2">
                                <svg xmlns="http://www.w3.org/2000/svg" width="2em" height="1.5em"
                                    ...
                                    <path fill="none" stroke="currentColor" stroke-linejoin="round"
                                        d="m5 4l4.5 3L14 4M2 8.5h5m-4 2h5m-3.5 2h10v-9h-10v3H1" />
                                </svg>

                                Welcome Email
                            </div>
                        </button>
                        <button wire:click="$set('openTabs', 'history')" @class([
                            'flex-1 rounded-xl px-5 py-3 text-sm font-semibold transition-all duration-300',
                ...
                @if ($openTabs === 'overview')
                    <section
                        class="bg-gray-100 text-black rounded-lg bg border dark:border-slate-700 p-4 mb-4 dark:bg-slate-800 dark:text-slate-100">
                ...
                @elseif ($openTabs === 'activities')
                    <section
                        class="bg-gray-100 text-black rounded-lg border dark:border-slate-700 p-4 mb-4 dark:bg-slate-800 dark:text-slate-100">
            ...
            dark:bg-slate-900 p-5 shadow-sm">
                            ...
                            <div class="space-y-3">
                                ...
                                @forelse($deals->getMedia('compliance_documents')
                    as $file)
                                    <div
            ...
            dark:bg-slate-900 p-5 shadow-sm">
                            ...
                            <div class="space-y-3">
                                ...
                                @forelse($deals->getMedia('contract_documents')
                    as $file)
                                    <div
                                        class="flex items-center justify-between
                        rounded-xl border border-slate-200
        ...
        <div
            class="fixed bg-white w-full py-5 bottom-0 right-0 flex justify-end mt-4 gap-5 px-10 border-t border-slate-200 dark:bg-slate-800 dark:border-slate-700">
            @if (session('success'))
                <span
                    class="self-center text-sm text-emerald-600 dark:text-emerald-400">{{ session('success') }}</span>
            @endif
            @if (session('info'))
                <span class="self-center text-sm text-slate-500 dark:text-slate-400">{{ session('info') }}</span>
            @endif
            <button class="rounded bg-slate-100 text-gray-600 px-4 py-2 dark:bg-slate-700 dark:text-slate-100"
